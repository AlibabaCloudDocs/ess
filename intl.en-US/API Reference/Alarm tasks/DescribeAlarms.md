...
|MetricType|String|No|system|The type of the monitoring metric. Valid values:
...
|MetricName|String|No|CpuUtilization|The name of the event-triggered task. |
...
-   state: the status of TCP connections |
|DimensionValue|String|eth0|The value of the dimension that is associated with the metric. The value of this parameter depends on the DimensionKey parameter.
 ...
 user\_id: automatically filled by the system.

 scaling\_group: automatically filled by the system.
...
-   eth1: the Internet NIC for classic network-type ECS instances.

 Valid values of state:

 -   TCP\_TOTAL: the total number of TCP connections
...
|MetricName|String|CpuUtilization|The name of the monitroing metric. Valid values:
...
-   VpcInternetRx: the inbound traffic over the Internet to the VPC. Unit: KB/min.
-   VpcInternetTx: the outbound traffic over the Internet from the VPC. Unit: KB/min.
...
|MetricType|String|system|The type of the monitoring metric. Valid values:
