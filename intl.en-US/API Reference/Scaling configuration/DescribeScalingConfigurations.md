...
|DeleteWithInstance|Boolean|true|Specifies whether to release the data disk when its attached instance is released. Valid values:
...
|PrivatePoolOptions.Id|String|eap-bp67acfmxazb4\*\*\*\*|The ID of the private pool. Set the value to the ID of the elasticity assurance or capacity reservation that generates the private pool.

 **Note:** This parameter is in invitational preview. For more information, submit a ticket. |
|PrivatePoolOptions.MatchCriteria|String|Open|The type of the private pool. After an elasticity assurance or a capacity reservation takes effect, a private pool is generated. You can select a private pool when you launch an instance. Valid values:

 -   Open: the open mode. In this mode, the system automatically selects a matching private pool of the open type to launch the instance. If no matching private pools exist, the public resource pool is used to launch the instance.
-   Target: the specified mode. In this mode, a specified private pool is used to launch the instance. If the specified private pool is unavailable, the instance fails to be launched.
...
|SchedulerOptions|Struct| |**Note:** This parameter is in invitational preview and not available. |
|ManagedPrivateSpaceId|String|testManagedPrivateSpaceId|**Note:** This parameter is in invitational preview and not available. |
