...
|----|-----------|----------|
|Auto Scaling|Auto Scaling is a service that automatically scales Elastic Compute Service \(ECS\) instances. You can configure the settings based on your business needs. When business loads increase, Auto Scaling automatically adds ECS instances to ensure computing power. When business loads decrease, Auto Scaling automatically removes ECS instances to reduce costs.|[What is Auto Scaling?](/intl.en-US/Product Introduction/What is Auto Scaling?.md)|
|scaling group|A scaling group is a group of ECS instances that are dynamically scaled based on the configured scenario. You can specify the minimum and maximum numbers of ECS instances in a scaling group, and associate Server Load Balancer \(SLB\) and ApsaraDB RDS instances with the scaling group.|[Overview](/intl.en-US/Scaling Group/Scaling group/Overview.md)|
...
|Server Load Balancer instance or SLB instance|Server Load Balancer \(SLB\) is a service that distributes network traffic on demand to a group of backend servers to increase the throughput of your applications. You can use SLB to prevent service interruptions caused by single points of failure \(SPOFs\) and improve the availability of applications.|[What is SLB?](/intl.en-US/Classic Load Balancer/Product Introduction/What is CLB?.md)|
|RDS instance|RDS is a stable and reliable online database service that supports elastic scaling. It supports mainstream database engines and offers a full range of database solutions, such as disaster recovery, backup, restoration, monitoring, and migration.|[What is ApsaraDB RDS?](/intl.en-US/Product Introduction/What is ApsaraDB RDS?.md)|
...
|[Overview](/intl.en-US/Scaling Group/Scaling rule/Overview.md)|
